...
export const AuraFarmerDetailSheet = ({
  auraFarmerId,
  name,
  description,
  rank,
    ...
    return (
      <div className="flex flex-col items-start px-8 pt-4">
        <img
          ...
          alt={`${name} avatar`}
          className="w-auto h-48 rounded-xl object-contain mb-4"
        />
        ...
        <SheetDescription className="max-w-[80ch]">{description}</SheetDescription>
      </div>
    );
  };
    ...
    <Sheet>
      ...
      <SheetContent side="right" className="min-w-full lg:min-w-[80vw]">
        ...
        <Tabs defaultValue="details" className="px-4">
          ...
          <TabsContent value="details">{getDetailTab()}</TabsContent>
